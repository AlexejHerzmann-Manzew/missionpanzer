/*
 * Copyright (C) 2014-2015 yew_mentzaki
 *
...
public final class FontRender {
    ...
    private FontRender(Font f) {
        this.font = f;
        this.f = f.deriveFont((f.getSize() * 2f));
        init();
        textRender.add(this);
    }
    ...
    public void drawString(String text, double x, double y, Color color, int maxWidth, boolean WordWrap) {
        if (text == null) {
            return;
        }
        color.bind();
        ...
        if (WordWrap) {
            ...
            for (String tx : text.split(" ")) {
                ...
                for (char c : tx.toCharArray()) {
                    ...
                    glVertex2d(x + t.getImageWidth() / 2, y);
                    glTexCoord2f(t.getWidth(), t.getHeight());
                    glVertex2d(x + t.getImageWidth() / 2, y + t.getImageHeight() / 2);
                    glTexCoord2f(0, t.getHeight());
                    glVertex2d(x, y + t.getImageHeight() / 2);
                    glEnd();
                    x += t.getImageWidth() / 2;
                }
                x += spacewidth;
